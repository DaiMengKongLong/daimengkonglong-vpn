            ...
            <div class="glass-card">
                ...
                <form method="POST" class="node-form">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label>节点类型</label>
                            <select name="nodeType" required onchange="toggleNodeFields(this.value)">
                        ...
                        <div class="form-group">
                            <label>节点名称</label>
                            <input type="text" name="nodeName" required>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label>服务器地址</label>
                            <input type="text" name="nodeServer" required>
                        </div>
                        
                        <div class="form-group">
                            <label>端口</label>
                            <input type="number" name="nodePort" required min="1" max="65535">
                        </div>
                    </div>
                    
                    <div id="vmess-fields" class="node-fields">
                        <div class="form-group">
                            <label>UUID</label>
                            <input type="text" name="nodeUuid">
                        </div>
                        
                        <div class="form-group">
                            <label>AlterID</label>
                            <input type="number" name="nodeAlterId" value="0">
                        </div>
                    </div>
                    
                    <div id="trojan-fields" class="node-fields" style="display: none;">
                        <div class="form-group">
                            <label>密码</label>
                            <input type="text" name="nodePassword">
                        </div>
                    </div>
                    
                    <div id="ss-fields" class="node-fields" style="display: none;">
                        <div class="form-row">
                            <div class="form-group">
                                <label>密码</label>
                                <input type="text" name="nodePassword">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="nodePath" placeholder="/path">
                        </div>
                        
                        <div class="form-group">
                            <label>Host</label>
                            <input type="text" name="nodeHost" placeholder="example.com">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label>SNI</label>
                        <input type="text" name="nodeSni" placeholder="example.com">
                    </div>
        ...
        <div id="proxy" class="tab-content">
            ...
            <div class="glass-card">
                ...
                <form method="POST" class="proxy-form">
                    <input type="hidden" name="action" value="add_proxy_ip">
                    
                    <div class="form-group">
                        <label>IP地址或域名</label>
                        <input type="text" name="proxyIP" required placeholder="1.2.3.4 或 example.com">
                    </div>
                    
                    <button type="submit" class="submit-btn">➕ 添加IP</button>
                </form>
            </div>
            ...
            <div class="glass-card">
                ...
                <form method="POST" class="import-form">
                    <input type="hidden" name="action" value="import_proxy_ips">
                    
                    <div class="form-group">
                        <label>导入数据 (支持TXT/CSV格式，每行一个IP)</label>
                        <textarea name="importData" rows="10" placeholder="1.2.3.4&#10;example.com&#10;5.6.7.8,443,备注"></textarea>
                    </div>
                    
                    <button type="submit" class="submit-btn">📥 批量导入</button>
                </form>
            </div>
        </div>
...
function getAdminJavaScript() {
  return '
    ...
    function toggleNodeFields(nodeType) {
        // 隐藏所有节点字段
        const allFields = document.querySelectorAll('.node-fields');
        allFields.forEach(field => {
            field.style.display = 'none';
        });

        // 显示对应类型的字段
        const targetField = document.getElementById(nodeType + '-fields');
        if (targetField) {
            targetField.style.display = 'block';
        }

        // 根据节点类型设置必填字段
        const uuidField = document.querySelector('input[name="nodeUuid"]');
        const passwordField = document.querySelector('input[name="nodePassword"]');
        const methodField = document.querySelector('select[name="nodeMethod"]');

        // 清除所有必填状态
        if (uuidField) uuidField.required = false;
        ...
        switch(nodeType) {
            case 'vmess':
            case 'vless':
                if (uuidField) uuidField.required = true;
                break;
            case 'trojan':
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 初始化节点字段显示
        const nodeTypeSelect = document.querySelector('select[name="nodeType"]');
        if (nodeTypeSelect) {
            toggleNodeFields(nodeTypeSelect.value);
        }
