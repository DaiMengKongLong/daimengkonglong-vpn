...
function generateHomePage(config, baseUrl, token) {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>' + (config.customTitle || '节点订阅服务') + '</title><style>' + getGlassmorphismCSS() + (config.customCSS || '') + '</style></head><body><div class="background"><div class="shape"></div><div class="shape"></div><div class="shape"></div></div><div class="container"><header class="glass-card"><div class="logo"><img src="' + (config.icon || 'https://img.picui.cn/free/2025/06/30/686234d353680.png') + '" alt="Logo" class="logo-img"><h1>' + (config.name || '节点订阅服务') + '</h1></div><p class="description">' + (config.description || '多格式订阅转换服务') + '</p></header><main class="main-content"><section class="subscription-section"><h2>📡 订阅链接</h2><div class="subscription-grid">' + subscriptionLinks.map(link => '<div class="subscription-card glass-card" onclick="copyToClipboard(\'' + link.url + '\')"><div class="subscription-icon">' + link.icon + '</div><h3>' + link.name + '</h3><p class="subscription-url">' + link.url + '</p><button class="copy-btn">📋 复制链接</button></div>').join('') + '</div></section><section class="config-section"><h2>⚙️ 配置信息</h2><div class="config-card glass-card"><div class="config-item"><span class="config-label">节点数量:</span><span class="config-value">' + (config.nodes ? config.nodes.length : 0) + '</span></div><div class="config-item"><span class="config-label">反代IP数量:</span><span class="config-value">' + (config.proxyIPs ? config.proxyIPs.length : 0) + '</span></div><div class="config-item"><span class="config-label">Token:</span><span class="config-value">' + token + '</span></div></div></section><section class="ini-section"><h2>📄 INI配置</h2><div class="ini-card glass-card"><textarea readonly class="ini-content">' + (config.iniTemplate || '') + '</textarea><button class="copy-btn" onclick="copyIniConfig()">📋 复制INI配置</button></div></section></main><footer class="glass-card"><p>© 2024 节点订阅服务 | <a href="' + baseUrl + '/admin?token=' + token + '" class="admin-link">🔧 管理面板</a></p></footer></div><div id="toast" class="toast"></div><script>' + getJavaScript() + '</script></body></html>';
