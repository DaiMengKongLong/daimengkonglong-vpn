...
function generateHomePage(config, baseUrl, token) {
  // 使用自定义图标或默认图标
  const icons = config.subscriptionIcons || {};
  const subscriptionLinks = [
    { name: 'Base64订阅', url: baseUrl + '/sub/base64?token=' + token, icon: icons.base64 || '📄' },
    { name: 'Clash订阅', url: baseUrl + '/sub/clash?token=' + token, icon: icons.clash || '⚔️' },
    { name: 'SingBox订阅', url: baseUrl + '/sub/singbox?token=' + token, icon: icons.singbox || '📦' },
    { name: 'Loon订阅', url: baseUrl + '/sub/loon?token=' + token, icon: icons.loon || '🌙' },
